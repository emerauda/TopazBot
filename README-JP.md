...
**TopazBot**は、TopazChatのRTSPストリームをDiscordのボイスチャンネルに中継する、高音質・低遅延のミュージックボットです。Linuxなどのサーバーで動作します。

**注意!!**

_「TopazBot」はMITライセンスで公開されていますが、「TopazChat」は商用利用が禁止されています。_

### ✨ 主な機能

- 🧠 **高品質**: ステレオ・高音質・低遅延のストリーム再生
- 🔒 **セキュア**: DAVE (Discord Audio Video Encryption) による音声のエンドツーエンド暗号化
- 🌐 **多フォーマット対応**: FFmpegによる柔軟なストリーム処理
...
最も簡単な方法です。以下のリンクからあなたのDiscordサーバーにBotを追加できます。
...
| `INPUT_IS_OPUS` | No | `0` | `1` = ソースが実際にOpusを配信している場合のみ無変換コピー（TopazChatはAACのため `0` を推奨。誤設定時は自動的に再エンコードへ切り替え） |
...
再生時には `${RTSP_SERVER_URL}/${streamkey}` というURLでストリームにアクセスします。
...
オープンソースコミュニティが学び・創造・インスピレーションの場であり続けられるのは、コントリビューションのおかげです。皆さまの貢献を**心より歓迎**します。

このプロジェクトを改善する提案があれば、リポジトリをフォークしてプルリクエストを作成してください。"enhancement" タグを付けてIssueを立てるだけでも歓迎です。
プロジェクトへのスターもお忘れなく！ありがとうございます！
